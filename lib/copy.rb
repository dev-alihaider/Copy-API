# Class to handle copy related data from airtable and formatting values
...
class Copy
  def refresh
    ...
    $file_data = data_hash # I am using global variable or else I need to load file on every request
  end
  ...
  def format_value(params)
    ...
    if file_data[params[:key]].present?
      ...
      params.each do |k, v|
        if "{#{k}}".in? value
          value.gsub!("{#{k}}", v)
        elsif "{#{k}, datetime}".in? value # we need to make this kind of checks for custom formatting as per requirements
          t = Time.at(v.to_i)
          value.gsub!("{#{k}, datetime}", t.strftime('%a %b %d %I:%M:%S%p')) # it may show different time than written in doc due to timezone difference
        end
      end
    end
    value
  ...
  def get_data(params)
    data = get_file_data
    data = data.select { |_k, v| v[:updated_at] >= params[:since].to_i } if params[:since].present?
    data
  end
